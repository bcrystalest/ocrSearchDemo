...
@property (nonatomic, strong)UIButton *heightLight;
@end

@implementation LHSIDCardScaningView

- (UIButton *)heightLight
{
    if (_heightLight == nil) {
        _heightLight = [[UIButton alloc]init];
    }
    return _heightLight;
}
...
-(void)addScaningWindow {
    ...
    // 提示标签
    CGPoint center2 = self.center;
    center2.y = CGRectGetMaxY(_IDCardScanningWindowLayer.frame) - _IDCardScanningWindowLayer.frame.size.height - 40;
    [self addLightWithTextWithCenter:center2];
...
#pragma mark - 添加闪光灯按钮
- (void)addLightWithTextWithCenter:(CGPoint)center {
    [self addSubview:self.heightLight];
    
//    heightLight.backgroundColor = [UIColor whiteColor];
    [_heightLight setImage:[UIImage imageNamed:@"开灯"] forState:UIControlStateNormal];
    [_heightLight addTarget:self action:@selector(showLight) forControlEvents:UIControlEventTouchDown];
//    _heightLight.layer.masksToBounds = YES;
//    _heightLight.layer.borderColor = [UIColor whiteColor].CGColor;
//    _heightLight.layer.cornerRadius = 4;
//    _heightLight.layer.borderWidth = 1;
    _heightLight.transform = CGAffineTransformMakeRotation(M_PI * 0.5);
    [_heightLight sizeToFit];
    
    _heightLight.center = center;
}
